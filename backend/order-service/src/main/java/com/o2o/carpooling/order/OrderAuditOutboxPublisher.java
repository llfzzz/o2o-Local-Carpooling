...
/**
 * Relays pending order audit-outbox entries to the audit service with retry.
 ...
 * Separate from {@link OrderOutboxPublisher} (RabbitMQ payment-timeout path).
 */
@Service
class OrderAuditOutboxPublisher {
    ...
    private static final Logger log = LoggerFactory.getLogger(OrderAuditOutboxPublisher.class);

    private final OrderAuditOutboxRepository repository;
    private final OrderAuditFeignClient auditFeignClient;
    private final ObjectMapper objectMapper = new ObjectMapper();

    OrderAuditOutboxPublisher(OrderAuditOutboxRepository repository, OrderAuditFeignClient auditFeignClient) {
        this.repository = repository;
        this.auditFeignClient = auditFeignClient;
    }

    @Scheduled(fixedDelayString = "${orders.audit-outbox.publish-fixed-delay:PT10S}")
    void publishDue() {
    ...
    int publishDue(Instant now) {
        int sent = 0;
        for (OrderAuditOutboxEntry entry : repository.findSendable(now, BATCH_SIZE)) {
            try {
