...
/**
 * Relays pending driver audit-outbox entries to the audit service with retry.
 ...
 */
@Service
class DriverAuditOutboxPublisher {
    ...
    private static final Logger log = LoggerFactory.getLogger(DriverAuditOutboxPublisher.class);

    private final DriverAuditOutboxRepository repository;
    private final DriverAuditFeignClient auditFeignClient;
    private final ObjectMapper objectMapper = new ObjectMapper();

    DriverAuditOutboxPublisher(DriverAuditOutboxRepository repository, DriverAuditFeignClient auditFeignClient) {
        this.repository = repository;
        this.auditFeignClient = auditFeignClient;
    }

    @Scheduled(fixedDelayString = "${drivers.audit-outbox.publish-fixed-delay:PT10S}")
    void publishDue() {
    ...
    int publishDue(Instant now) {
        int sent = 0;
        for (DriverAuditOutboxEntry entry : repository.findSendable(now, BATCH_SIZE)) {
            try {
