...
- `0.4.0-SNAPSHOT` 起 Users、Driver Verification、Trips、Orders、Order Outbox、Payment Sim、Files、AI OCR Mock 任务由各服务自己的 Flyway migration 建表。
...
- Trip 库存由 `trips.locked_seats` 和 `trip_seat_locks.order_id` 共同维护；Order 使用 `(rider_id, idempotency_key)` 防重复下单；Payment Sim 使用 `(order_id, idempotency_key)` 防重复支付模拟；文件上传状态由 `file_objects.upload_status` 标识。
