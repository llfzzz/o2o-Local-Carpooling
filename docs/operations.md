...
## 性能与可扩展性基线（Round 1，2026-07-23）

第一轮性能优化只做「安全、可单测验证、可配置回滚、零新增 Redis/JVM 占用」的改动，不改 API 契约与安全语义。要点：

- **Redis 永不淘汰安全状态**：本项目 Redis 里的每个 key 都是带 TTL 的安全/正确性状态（短信码+尝试计数、refresh token、支付回调 nonce、限流窗口、司机在线位置），没有可淘汰的缓存。`maxmemory-policy` 必须是 `noeviction`——基础 `docker-compose.yml` 用 Redis 默认值（即 noeviction），低内存 overlay `docker-compose.lowmem.yml` 显式钉死。超内存时写入应「明确失败并被监控发现」，绝不能静默淘汰。按环境把 `maxmemory` 调到真实工作集。
- **Hikari 连接池显式化**：每个落库服务的 `application.yml` 现在显式声明 `spring.datasource.hikari.*`（`HIKARI_*` 环境变量可覆盖），不再依赖启动参数——否则不带参数直接 `java -jar` 会悄悄回到 10 连接默认值。低内存 systemd profile 仍是 `maximum-pool-size=2`。
- **Feign 读超时收敛到 30s**（原 90s，`deploy/systemd/o2o@.service`）：下游卡死时更快释放调用线程（Hikari 2 连接 + 单核下，几个 90s 卡顿就会耗尽池）。30s 仍高于实测最差冷启动 ~14.5s 约 2 倍；某服务需要更长可用其 `EnvironmentFile` 覆盖。
- **限流按真实客户端 IP**：网关此前只读 socket peer，nginx 后所有匿名流量共用「代理 IP」一个桶。现在受信代理（默认 loopback 上的 nginx，可加 `security.rate-limit.trusted-proxies`）会读 `X-Real-IP`/`X-Forwarded-For` 取真实客户端 IP；429 响应带 `Retry-After`；内存限流器超过阈值会清理过期窗口，不再无界增长。仍是内存态（单网关够用），分布式限流留待网关横向扩展时再做。
- **可观测性**：`micrometer-registry-prometheus` 已加入（根 `backend/pom.xml`，覆盖全部服务），`/actuator/prometheus` 从「404」变为可抓取，附带 JVM/GC/线程/HikariCP/HTTP 内建指标。自定义低基数指标：网关限流决策 `gateway.ratelimit.decisions{bucket,outcome}`、座位锁对账 `order.seatlock.reconciliation{outcome}`（标签绝不含用户/IP/手机号）。自检：`curl -s http://127.0.0.1:8120/actuator/prometheus | head`（网关）或各服务端口。
- **座位库存对账兜底**：`order-service` 新增 `@Scheduled` `reconcileCancelledSeatLocks`——对最近取消（`ORDERS_SEAT_RECONCILIATION_LOOKBACK`，默认 30min）的订单重放幂等的 `releaseSeats`，修复「订单已取消但 releaseSeats Feign 调用失败导致座位仍被锁」的极小概率漂移；`releaseSeats` 幂等（已 RELEASED 直接返回，不二次扣减），只会释放真漏掉的锁，绝不会超卖。反向漂移（已锁座订单丢锁）不自动修复（重锁可能超卖），仅留指标待后续。

### SQL 索引/查询改动的 EXPLAIN 取证（待运行态补齐）

本轮的索引与查询改动（`idx_orders_created`、`idx_trips_geo_seek`、`idx_notif_user_id_seq`、admin 列表加界、司机审核数改为索引 `count(*)`、outbox 排序对齐索引、用户 upsert 改幂等）已由各仓库 H2 切片测试验证「结果正确、行为不变」，但 **before/after `EXPLAIN ANALYZE` 证据需要一个已迁移+已灌数的 MySQL**。本轮会话中本机 Docker demo 栈未启动、原生 MySQL 凭据未知（不猜测、不改动用户库），故未生成，属如实记录的缺口。取到已灌数的非生产 MySQL 后按此取证：

```sql
-- admin 今日订单计数（应命中 idx_orders_created，不再全表扫）
EXPLAIN ANALYZE SELECT count(*) FROM orders WHERE created_at >= '2026-07-23 00:00:00';
-- 邻近搜索（对比 idx_trips_geo vs idx_trips_geo_seek 的 rows examined）
EXPLAIN ANALYZE SELECT * FROM trips WHERE status='PUBLISHED'
  AND origin_lat BETWEEN 24.4 AND 24.6 AND origin_lng BETWEEN 118.0 AND 118.2;
-- 收件箱 keyset 分页（应命中 idx_notif_user_id_seq，无 filesort）
